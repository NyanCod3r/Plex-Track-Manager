...
def _load_json_playlists(plex_json_path: Optional[str]) -> list:
    if not plex_json_path or not os.path.exists(plex_json_path):
        return []
    try:
        with open(plex_json_path, encoding="utf-8") as fh:
            data = json.load(fh)
        logging.info(f"[LB] Loaded {len(data)} playlists from {plex_json_path}")
        return data
    except Exception as exc:
        logging.warning(f"[LB] Could not load plex_playlists.json at '{plex_json_path}': {exc}")
        return []
...
def _load_plex_playlists(plex) -> list:
    ...
    try:
        ...
        for pl in plex.playlists():
            ...
            if tracks:
                result.append({"title": pl.title, "items": tracks})
        logging.info(f"[LB] Loaded {len(result)} audio playlists from Plex")
        return result
    except Exception as exc:
        logging.warning(f"[LB] Could not load live Plex playlists: {exc}")
        return []


def sync_playlists_to_lb(plex, plex_json_path: Optional[str], lb_token: str, lb_username: str) -> dict:
    """
    Sync all Plex playlists (from JSON + live Plex) to ListenBrainz.
    Creates new playlists and adds missing tracks to existing ones.
    Returns a summary dict with counts.
    """
    summary = {"created": 0, "updated": 0, "skipped": 0, "tracks_added": 0, "errors": 0}
    ...
    if not merged:
        logging.info("[LB] No playlists found to sync.")
        return summary

    logging.info(f"[LB] Syncing {len(merged)} playlists to ListenBrainz...")

    try:
        existing = {pl["title"]: pl["mbid"] for pl in get_lb_playlists(lb_username, lb_token)}
    except Exception as exc:
        logging.error(f"[LB] Failed to fetch existing LB playlists: {exc}")
        summary["errors"] += 1
        return summary
    ...
    for pl in merged:
        ...
        try:
            if name in existing:
                ...
                if missing:
                    add_tracks_to_lb_playlist(mbid, lb_token, missing)
                    summary["updated"] += 1
                    summary["tracks_added"] += len(missing)
                    logging.info(f"[LB] Updated '{name}': added {len(missing)} tracks")
                else:
                    summary["skipped"] += 1
                    logging.debug(f"[LB] Skipped '{name}': all {len(tracks)} tracks already present")
            else:
                mbid = create_lb_playlist(lb_token, name, tracks)
                if mbid:
                    summary["created"] += 1
                    summary["tracks_added"] += len(tracks)
                    logging.info(f"[LB] Created '{name}' with {len(tracks)} tracks")
                else:
                    logging.error(f"[LB] Failed to create playlist '{name}': no mbid returned")
                    summary["errors"] += 1
        except requests.HTTPError as exc:
            logging.error(f"[LB] HTTP {exc.response.status_code} syncing '{name}': {exc}")
            summary["errors"] += 1
        except Exception as exc:
            logging.error(f"[LB] Error syncing playlist '{name}': {exc}")
            summary["errors"] += 1

    logging.info(
        f"[LB] Sync complete: {summary['created']} created, {summary['updated']} updated, "
        f"{summary['skipped']} skipped, {summary['tracks_added']} tracks added, "
        f"{summary['errors']} errors"
    )
    return summary


def remove_track_from_all_lb_playlists(artist: str, title: str, lb_token: str, lb_username: str) -> bool:
    """
    Search all LB playlists of the user and remove the track wherever it appears.
    Returns True if the operation completed without API errors.
    Returns False if any removal call failed (signals a CRITICAL cleanup failure).
    """
    try:
        playlists = get_lb_playlists(lb_username, lb_token)
    except Exception as exc:
        logging.error(f"[LB] Failed to fetch playlists during 1-star cleanup: {exc}")
        return False
    ...
    for pl in playlists:
        try:
            ...
            logging.error(
                f"[LB] HTTP {exc.response.status_code} removing '{artist} - {title}' "
                f"from '{pl['title']}': {exc}"
            )
            all_ok = False
        except Exception as exc:
            logging.error(f"[LB] Error removing '{artist} - {title}' from '{pl['title']}': {exc}")
            all_ok = False
